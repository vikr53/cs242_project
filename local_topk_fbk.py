...
if rank == 0:
    ...
    train_log_dir = 'logs/grad_dist_fbe_lin_topk'+str(k)+"_"+str(rank)+'/' + current_time + '/train'
    grad_var_log_dir = "logs/grad_dist_fbe_lin_topk"+str(k)+"_"+str(rank)+"/"+current_time + '/grad_var'
    test_log_dir = 'logs/grad_dist_fbe_lin_topk'+str(k)+"_"+str(rank)+'/' + current_time + '/test'
    ...
    for epoch in range(num_epoch):
        ...
        for step, (x_batch_train, y_batch_train) in enumerate(train_dataset):
            with tf.GradientTape() as tape:
                ...
                if step == 0 and (epoch == 0 or epoch == 14 or epoch == 28 or epoch == 42 or epoch == 56 or epoch == 69):
                    ...
                    # log grad_val as csv
                    log_grad = pd.DataFrame(concat_grads.numpy())
                    log_grad.to_csv('gradout'+str(k)+str(epoch), index=False)
                    
                # Compute top-k of u
                #k = 600
                k0 = 6000
                kf = 6
                k = int(k0 + math.floor((epoch)*(kf-k0)/(num_epoch)))
                #k=60000
