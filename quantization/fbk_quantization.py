...
if rank == 0:
    ...
    exp_name = "_quant_16_"
    train_log_dir = 'logs/fbe_'+str(rank)+'/' +str(exp_name)+ current_time + '/train'
    grad_var_log_dir = "logs/fbe_"+str(rank)+"/"+str(exp_name)+current_time + '/grad_var'
    test_log_dir = 'logs/fbe_'+str(rank)+'/'+str(exp_name) + current_time + '/test'
    ...
    for epoch in range(num_epoch):
        ...
        for step, (x_batch_train, y_batch_train) in enumerate(train_dataset):
            with tf.GradientTape() as tape:
                ...
                comm.send(q_np_u, dest=0, tag=11)
